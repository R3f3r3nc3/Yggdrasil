var systemComponents = {
	...
	'tree-node': {
      ...
      v-if="tmp[layer].tooltip != ''"
			:text="(tmp[layer].isLayer) ? (
				...
				: (tmp[layer].tooltipLocked ? tmp[layer].tooltipLocked : 'Unlock Requirement : ' + + formatWhole(tmp[layer].baseAmount) + '/' + formatWhole(tmp[layer].requires) + ' ' + tmp[layer].baseResource)
			)
			: (
	...
	'overlay-head': {
		...
		<br>
		<span v-if="canGenPoints()"  class="overlayThing">({{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getPointGen(),3)}}/sec)</span>
    ...
    'back-button': {
        template: `
        <button v-bind:class="back" onclick="goBack()">"Go back!!"</button>
        `
    },
